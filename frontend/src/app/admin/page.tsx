...
export default function AdminPage() {
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
      ...
      <p className="text-slate-400 mb-10">Mundial 2026 · Gestión de tu país</p>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {CARDS.map(({ href, icon: Icon, title, desc, color }) => (
          <Link key={href} href={href}>
            <Card className="hover:border-slate-500 hover:bg-slate-700/50 transition-all cursor-pointer group h-full">
              <CardContent className="p-6 flex flex-col gap-4">
                <div className={`p-3 rounded-lg bg-slate-700/50 w-fit ${color}`}>
